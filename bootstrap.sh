...
if [[ $(aws s3api head-bucket --bucket "$TF_S3_BUCKET" 2>/dev/null; echo $?) != 0 ]]; then
	echo "[INFO] The $TF_S3_BUCKET Bucket is not exists, Let's create it"
	aws s3api create-bucket --bucket $TF_S3_BUCKET --region us-east-1
